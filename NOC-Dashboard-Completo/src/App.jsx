...
function DashboardRouter() {
  ...
  // Motor do Videowall: alterna as rotas a cada 5 segundos (pausado na aba de gerenciamento do banco)
  useEffect(() => {
    if (carregando) return;
    if (location.pathname === '/banco-de-dados') return;
    if (tempoRestante > 0) {
  ...
  }, [tempoRestante, location.pathname, navigate, carregando]);

  // Tela de Loading enquanto o Back-End processa a resposta
  if (carregando) return (
        ...
        <div className="container-fluid flex-column align-items-start px-3 py-2">
          <div className="d-flex w-100 justify-content-between align-items-center mb-3">
            <span className="navbar-brand fw-bold text-info m-0 d-flex align-items-center">
              {/* Globo apontando para a matriz do NOC, com coordenada vinda do SQLite */}
              <a
            ...
            <span className="badge bg-transparent border border-info text-info px-3 py-2">
              {location.pathname === '/banco-de-dados' ? '🛠️ MODO GERENCIAMENTO' : `AUTO-SWAP: 00:0${tempoRestante}`}
            </span>
          </div>
          ...
          <div className="nav-scroll w-100 gap-2">
            ...
            <Link to="/banco-de-dados" onClick={() => setTempoRestante(5)} className={`btn btn-sm text-nowrap px-3 py-2 ${location.pathname === '/banco-de-dados' ? 'btn-warning text-dark fw-bold shadow' : 'btn-outline-warning text-white'}`}>
              🗄️ Banco de Dados
            </Link>
          </div>
        </div>
